...
class PlaybookParser {
  ...
  _processBlock(lines) {
    ...
    while (i < trimmedLines.length) {
      ...
      // Detect markdown image syntax ![alt text](path/to/image.png)
      // Check if it's the Smart Calling Cheat Sheet - make it full-page
      const markdownImageMatch = line.match(/^!\[([^\]]*)\]\(([^)]+)\)$/);
      if (markdownImageMatch) {
        const imagePath = markdownImageMatch[2].trim();
        // If it's the Smart Calling Cheat Sheet, make it full-page
        if (imagePath.toLowerCase().includes('smart calling cheat sheet')) {
  ...
  toHTML(parsed) {
    ...
    normalizedSections.forEach((section, idx) => {
      ...
      section.subsections.forEach((subsection, subIdx) => {
        // Check if this subsection contains a full-page image
        const fullPageImageItem = subsection.content.find(item => item.type === 'fullpageImage');
        
        // If it has a full-page image, render it on its own page first
        if (fullPageImageItem) {
          // Close current page-content and page
          html += '</div>\n'; // Close page-content
          html += '<div class="page-number">' + (2 + idx + 1) + '</div>\n';
          html += '</div>\n'; // Close page
          
          // Create new dedicated page for the full-page image
          html += `<div class="page" id="${section.id}-${subsection.number}-fullpage">\n`;
          
          // Render subsection title first
          html += `<h3>${section.number}.${subsection.number} ${this._escapeHtml(subsection.title)}</h3>\n`;
          
          // Render the full-page image
          html += this._renderContentItem(fullPageImageItem);
          
          // Render any caption text that comes after the image
          subsection.content.forEach(item => {
            if (item.type === 'paragraph' && item !== fullPageImageItem) {
          ...
          // Add page number
          html += '<div class="page-number">' + (2 + idx + 1) + '</div>\n';
          html += '</div>\n'; // Close full-page image page
          
          // Create a new page for remaining subsections
          html += `<div class="page" id="${section.id}-${subsection.number}-cont">\n`;
          html += '<div class="page-content">\n';
          
          // Continue with next subsections - this one is done
          return;
        }
        ...
        subsection.content.forEach((item, idx) => {
          // Skip full-page images in normal flow (they're handled above)
          if (item.type === 'fullpageImage') {
            return;
          }
