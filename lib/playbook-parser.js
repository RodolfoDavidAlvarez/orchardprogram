...
class PlaybookParser {
  ...
  toHTML(parsed) {
    ...
    // Table of Contents
    html += this._renderTOC(parsed.toc);
    ...
    parsed.sections.forEach((section, idx) => {
      ...
      html += `<div class="page-number">${section.number + 2}</div>\n`;
      html += '</div>\n';
    });
  ...
  _renderTOC(toc) {
    ...
    toc.forEach(section => {
      ...
      html += `<a href="#section${section.number}">${section.number}. ${this._escapeHtml(section.title)} <span>Page ${section.number + 2}</span></a>\n`;
