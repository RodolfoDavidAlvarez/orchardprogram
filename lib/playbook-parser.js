...
class PlaybookParser {
  ...
  _replaceEmojisWithIcons(text) {
    ...
    // Keyword-based icons (adds crop icons even without emoji)
    const keywordIconMap = [
      { regex: /\bavocados?\b/gi, icon: '<i class="fas fa-leaf" style="color: #27ae60;"></i>' },
      { regex: /\bcitrus\b/gi, icon: '<i class="fas fa-lemon" style="color: #f1c40f;"></i>' },
      { regex: /\bpistachios?\b/gi, icon: '<i class="fas fa-seedling" style="color: #9b59b6;"></i>' },
      { regex: /\bvines?\b|\bvineyards?\b/gi, icon: '<i class="fas fa-wine-bottle" style="color: #8e44ad;"></i>' },
      { regex: /\bpome\b/gi, icon: '<i class="fas fa-apple-alt" style="color: #e74c3c;"></i>' },
      { regex: /\bstone\s+fruits?\b/gi, icon: '<i class="fas fa-seedling" style="color: #f39c12;"></i>' }
    ];

    let result = String(text);
    keywordIconMap.forEach(({ regex, icon }) => {
      result = result.replace(regex, (match) => `${match} ${icon}`);
    });
