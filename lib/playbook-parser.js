...
class PlaybookParser {
  ...
  parse() {
    ...
    while (i < lines.length) {
      ...
      if (sectionMatch && nextLine.match(/^=+$/)) {
        ...
        // Skip section 2 (MARKETING/SALES PIPELINE OVERVIEW)
        if (sectionNum === 2) {
          currentSection = null;
      ...
      // Regular content - skip if we're in section 2 (currentSection is null)
      if (currentSection === null) {
        // We're in section 2, skip all content until next section
        i++;
        continue;
      }
  ...
  toHTML(parsed) {
    ...
    // Filter out section 2 (MARKETING/SALES PIPELINE OVERVIEW) from sections
    const filteredSections = parsed.sections.filter(section => section.number !== 2);
    
    // Generate TOC from parsed sections (not from TOC text section)
    const generatedTOC = this._generateTOCFromSections(filteredSections);
    
    // Table of Contents
    html += this._renderTOC(generatedTOC);

    // Sections (excluding section 2)
    // Each section starts on a new page, content flows naturally
    filteredSections.forEach((section, idx) => {
      ...
      const adjustedPageNumber = 2 + idx + 1; // Cover (1) + TOC (2) + section index
